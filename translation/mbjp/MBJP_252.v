...
{ eapply T_ProgramConcat.
  ...
  { eapply T_ProgramConcat.
    ...
    { eapply T_ProgramConcat.
      ...
      { eapply T_ClassDecl with(name:="Convert").
        { simpl. try reflexivity. }
        { eapply T_MethodDecl with(modif:="public static")(T:=(TyGeneric1 "List" TyDouble))(m:="convert").
          { simpl. try reflexivity. }
          { eapply T_Concat.
            { eapply T_DeclNoInit with(T:=TyInt)(x:="numbers").
              simpl. try reflexivity. }
            { eapply T_Skip. } }
          { simpl. try reflexivity. }
          { eapply T_Concat.
                  ...
                  { eapply T_Concat.
                    { eapply T_Return.
                      { eapply T_TyMethodInvocation with(m:="asList")(T:=(TyExternal "Arrays")).
                        ...
                        { simpl. try reflexivity. }
                        { eapply T_List.
                          { eapply T_Float with(f:=5.0%float). }
                          { eapply T_List.
                            { eapply T_Float with(f:=0.0%float). }
                            { eapply T_Nil. } } }
                        { simpl. try reflexivity. } }
