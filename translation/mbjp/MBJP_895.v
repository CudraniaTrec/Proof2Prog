...
{ eapply T_ProgramConcat.
  ...
  { eapply T_ProgramConcat.
    ...
    { eapply T_ProgramConcat.
      ...
      { eapply T_ClassDecl with(name:="MaxSumSubseq").
        { simpl. try reflexivity. }
        { eapply T_MethodDecl with(modif:="public static")(T:=TyInt)(m:="maxSumSubseq").
          { simpl. try reflexivity. }
          { eapply T_Concat.
          ...
          { eapply T_Concat.
            ...
            { eapply T_Concat.
              ...
              { eapply T_Concat.
                { eapply T_Concat.
                  { eapply T_DeclInit with(T:=TyInt)(x:="prevSum").
                    { simpl. try reflexivity. }
                    { eapply T_Integer with(n:=0). }
                    { simpl. try reflexivity. } }
                  { eapply T_Concat.
                    { eapply T_DeclInit with(T:=TyInt)(x:="currSum").
                      { simpl. try reflexivity. }
                      { eapply T_Integer with(n:=0). }
                      { simpl. try reflexivity. } }
                    { eapply T_Skip. } } }
                { eapply T_Concat.
                  { eapply T_For.
                    ...
                    { eapply T_Concat.
                      { eapply T_Expression.
                        eapply T_Assign.
                        { eapply T_Var with(x:="currSum").
                          simpl. try reflexivity. }
                        { eapply T_Add.
                          { eapply T_Var with(x:="prevSum").
                            simpl. try reflexivity. }
