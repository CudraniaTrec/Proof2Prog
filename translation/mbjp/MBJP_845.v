...
{ eapply T_ProgramConcat.
  ...
  { eapply T_ProgramConcat.
    ...
    { eapply T_ProgramConcat.
      ...
      { eapply T_ClassDecl with(name:="FindDigits").
        { simpl. try reflexivity. }
        { eapply T_MethodDecl with(modif:="public static")(T:=TyInt)(m:="findDigits").
          { simpl. try reflexivity. }
          { eapply T_Concat.
                  ...
                  { eapply T_DeclInit with(T:=TyDouble)(x:="x").
                    ...
                    { eapply T_Add.
                      { eapply T_Mul.
                        { eapply T_Var with(x:="n").
                          simpl. try reflexivity. }
                        { eapply T_TyMethodInvocation with(m:="log10")(T:=(TyExternal "Math")).
                          { simpl. try reflexivity. }
                          { simpl. try reflexivity. }
                          { eapply T_List.
                            { eapply T_Div.
                              { eapply T_Var with(x:="n").
                                simpl. try reflexivity. }
                              { eapply T_TyFieldAccess with(f:="E")(T:=(TyExternal "Math")).
                                { simpl. try reflexivity. }
                                { simpl. try reflexivity. } } }
                            { eapply T_Nil. } }
                          { simpl. try reflexivity. } } }
